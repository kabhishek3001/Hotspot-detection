...
def get_voice_command():
    ...
    print("Listening for voice command ('Open' or 'Close')...")
    with mic as source:
    ...
    try:
        ...
        if "open" in command:
            ...
            print("Unrecognized command. Please say 'Open' or 'Close'.")
            return None
    ...
    try:
        ...
        while True:
            try:
                ...
                print("2. Voice command ('Open' or 'Close')")
    
                method = input("Enter 1 for manual or 2 for voice (or 'exit' to quit): ").strip().lower()
                if method == 'exit':
                    break
                ...
                if method == '1':
                    ...
                    print("Say 'Open', 'Close', or 'Exit' to quit voice command mode.")
                    while True:
                        voice_cmd = get_voice_command()
                        if voice_cmd == "open":
                            print("Opening...")
                            send_angles_to_arduino(arduino, 0, 0)
                        elif voice_cmd == "close":
                            print("Closing...")
                            send_angles_to_arduino(arduino, 180, 180)
                        elif voice_cmd is not None and "exit" in voice_cmd:
